...
- **Anything from this session.** Reproduced on the bundle from before it.
- **Which world.** Last Five, The Ninth Archive and Hush House all crash.
- **Which entry point.** A shelf card and the hero carousel both crash.
- **Navigation generally.** Discover ↔ Profile ↔ Library is fine, repeatedly.
- **Reanimated, LayoutAnimation, removeClippedSubviews.** The app uses none of
  them, and they are the usual suspects for a Fabric mounting crash.
- **Stack exhaustion, probably.** The repeated frames suggested it, but the
  fault address (0x406b48) is low — that reads as a null dereference with an
  offset rather than a stack guard page.

A `--dev true` bundle was tried, to see whether development mode would surface
a JS error behind it. It does not boot without Metro attached, so that avenue
needs the dev server rather than a swapped bundle.

## The one experiment that matters next

**Open a world on the physical iPhone.** `devicectl` currently lists it as
`unavailable`, so it could not be done here. The previous session played ten
turns of Itachi on that phone, which means that build reached a session — so
either this is specific to the simulator runtime, or it is a regression since,
and those two answers lead to completely different work.
