...
export const rails = {
  ...
  /**
   * `Top ranked` — le rayon des mieux notés.
   *
   * `Les mieux notés` plutôt que `Meilleur classement` : un rayon français
   * nomme les choses, pas le classement. Et pas `Top` tout court, qui est de
   * l'anglais qu'on entend partout sans qu'il veuille dire quoi que ce soit.
   */
  'rail.top_ranked': 'Les mieux notés',
} as const;
